...
function RecipeDetail({ recipe }) {
    return (
        <>
            <div>
                {/* Header */}
                <section className="flex flex-col h-screen items-center justify-between bg-cover" style={{ backgroundImage: `url(${recipe.imgs})` }}>
                    ...
                    <div className="w-full bg-gray-100 bg-opacity-50 h-60 flex flex-col items-center justify-center">
                        <h1 className="text-center text-9xl text-white bg-100 font-semibold tracking-widest">{recipe.name}</h1>
                    </div>
                ...
                <section className="bg-main-purple text-white py-12 break-words">
                    <article className="container mx-auto">
                        <h1 className="text-center text-6xl mt-8 mb-20">How To Make {recipe.name}</h1>
                        <ul className="list-disc list-outside ml-20 mt-8 text-3xl ">
                            {recipe.howto.map((stepItem) => <li key={crypto.randomUUID()}>{stepItem}</li>)}
                        </ul>
                    </article>
                </section>
            </div>
